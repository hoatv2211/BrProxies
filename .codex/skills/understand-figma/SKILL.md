...
3. Resolve `PLUGIN_ROOT` and ensure core is built (same logic as `/understand` Phase 0.1.5, checking `$PROJECT_ROOT/.codex/understand-anything/understand-anything-plugin` first for this project-local install). If `packages/core/dist/figma/index.js` is missing, run:
   ```bash
