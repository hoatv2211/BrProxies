...
3. Find the dashboard code. The dashboard is at `packages/dashboard/` relative to this plugin's root directory. Check these paths in order and use the first that exists:
   ...
   - `$PROJECT_DIR/.codex/understand-anything/understand-anything-plugin/packages/dashboard/` (project-local Codex install)
   ...
   for candidate in \
     ...
     "$PROJECT_DIR/.codex/understand-anything/understand-anything-plugin" \
     "$SELF_RELATIVE" \
   ...
   if [ -z "$PLUGIN_ROOT" ]; then
     ...
     echo "  - $PROJECT_DIR/.codex/understand-anything/understand-anything-plugin"
