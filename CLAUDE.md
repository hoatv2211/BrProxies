...
ShardX Launcher is an anti-detect browser built with Tauri (Rust + React/TypeScript). It provides browser fingerprint spoofing, SOCKS5 proxy integration with UDP relay, and multiple automation interfaces (HTTP API, MCP server, Python/Node SDKs).
...
├── sdks/
│   ├── node/         # @proxyshard/shardx (Node.js SDK)
│   └── python/       # shardx (Python SDK)
...
- Stored under: `%APPDATA%\shardx-launcher\` (win), `~/Library/Application Support/shardx-launcher/` (mac), `~/.config/shardx-launcher/` (linux)
